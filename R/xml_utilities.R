...
prior_xml <- function(x, jive ,vari = c("Mean", "LogVar"), treeid, spnames){
  ...
  if(grepl("OU", model) | grepl("BM", model)){
    xml_add_child(anc, "tree", idref="Tree.t:Species")
  }
  
  # traits
  traits <- xml_add_child(anc, "oneTraitData",id=sprintf("Jive%sStash", vari), spec="contraband.OneValueContTraits", pNames=paste(spnames, collapse = ","), nTraits="1")
  xml_add_child(traits, "traitValues",ideref = sprintf("Jive%s", vari))
  ...
  if(grepl("BM", model)){
    ...
    if(grepl("sigma", model)){
      mod <- xml_child(rate, "branchRateModel", id=sprintf("Jive%sCatClock", vari), spec="contraband.RandomLocalColorModel", scaling="false", includeRoot="true")
  ...
  if(grepl("OU", model)){
    ...
    if(any(model %in% "theta")){
      mod <- xml_child(optimum, "branchRateModel", id=sprintf("Jive%sCatClock", vari), spec="contraband.RandomLocalColorModel", scaling="false", includeRoot="true")
      xml_add_child(mod, "indicators", id="JiveShiftIndicators", spec="parameter.BooleanParameter", value=paste(rep("true", nreg), rep("false", 2*length(spnames)-2-nreg), collapse = " "))
      if(jive$hprior$ou.the.1(NA)[[2]][[1]] == "Uniform"){ # Uniform hyperprior must be specified here
        xml_add_child(mod, "colors", id=sprintf("Jive%sTheta", vari), spec="parameter.RealParameter",
                      ...
                      value=paste(ifelse(grepl("root", model), jive$init$ou.the[1], jive$init$ou.the[2]) ,collapse = " "))
      } else {
      ...
      if(jive$hprior$ou.the.1(NA)[[2]][[1]] == "Uniform"){ # Uniform hyperprior must be specified here
        ...
        xml_add_child(mod, "rates", id=sprintf("Jive%sTheta", vari), spec="parameter.RealParameter", ifelse(grepl("root", model), jive$init$ou.the[1], jive$init$ou.the[2]))
      }
      xml_add_child(mod, "rateCatAssign", id=sprintf("Jive%sAssignments", vari), spec="parameter.IntegerParameter", lower="0", upper="0", rep(0, 2*length(spnames) - 1))
      xml_add_child(mod, "tree", idref=treeid)
    }
  ...
  # parameters
  if(grepl("WN", model)){
    if(jive$hprior$wn.sig[[2]][[1]] == "Uniform"){
      xml_add_child(anc, "SigmaSqs", id = sprintf("Jive%sEvolRate", vari), spec="parameter.RealParameter",
                    lower = jive$hprior$wn.sig[[2]][[1]][1],
                    upper = jive$hprior$wn.sig[[2]][[1]][2],
                    jive$init$wn.sig)
    } else {
  ...
  if(grepl("OU", model)){
    xml_add_child(anc, "rootState", id=sprintf("Jive%sRootValue", vari), dimension="1", spec="parameter.RealParameter", jive$init$ou.the[1])
    xml_add_child(anc, "alpha", id=sprintf("Jive%sAlpha", vari), dimension="1", spec="parameter.RealParameter", jive$init$ou.alp)
...
operator_xml <- function(x, jive ,vari = c("Mean", "LogVar"), treeid, spnames){
  ...
  if(vari == "lik"){
    xml_add_sibling(op, "operator", id = "JiveLogVars", spec = "RealRandomWalkOperator", parameter = "@JiveLogVars", windowSize = as.character(mean(jive$ws$v.sp)), weight = "20.0", .where = "before")
    xml_add_sibling(op, "operator", id = "JiveMeans", spec = "RealRandomWalkOperator", parameter = "@JiveMeans", windowSize = as.character(mean(jive$ws$m.sp)), weight = "20.0", .where = "before")
  } else {
    model <- jive$name
    nreg <- grep("\\[(.*?)\\]", model)
    ...
    if(grepl("OU", model)){
      xml_add_sibling(op, "operator", id = sprintf("Jive%sThetaRandomWalk", vari), spec = "RealRandomWalkOperator", parameter = sprintf("@Jive%sTheta", vari), scaleFactor = as.character(jive$ws$ou.the.1), weight = "20.0", .where = "before")
      if(grepl("theta", model)){
        shift <- xml_add_sibling(op, "operator", id = sprintf("Jive%sShiftIndicatorsShiftIndicatorMove", vari), spec = "contraband.operators.BitMoveOperator",  weight = "40.0", parameter = sprintf("@Jive%sShiftIndicators", vari), k = as.character(nreg), .where = "before")
        xml_add_child(shift, "tree", idref = treeid)
      }
      xml_add_sibling(op, "operator", id = sprintf("Jive%sAlphaScaler", vari), spec = "ScaleOperator", parameter = sprintf("@Jive%sAlpha", vari), scaleFactor = as.character(jive$ws$ou.alp.1), weight = "3.0", .where = "before")
      xml_add_sibling(op, "operator", id = sprintf("Jive%sEvolRateScaler", vari), spec = "ScaleOperator", parameter = sprintf("@Jive%sEvolRate", vari), scaleFactor = as.character(jive$ws$ou.sig.1), weight = "3.0", .where = "before")
      if(grepl("root", model)){
        xml_add_sibling(op, "operator", id = sprintf("Jive%sRootValueRandomWalk", vari), spec = "RealRandomWalkOperator", parameter = sprintf("@Jive%sRootValue", vari), windowSize = as.character(jive$ws$ou.the.0), weight = "20.0", .where = "before")
      }
    }
  }
}
