...
OUwie.sim2 <- function(phy, data=NULL, simmap.tree=FALSE, scaleHeight=FALSE, alpha, sigma.sq, theta0, theta){
	...
	sim.dat
	return(list(x,sim.dat))
}


#result<-OUwie.sim2(tree,trait,simmap.tree=FALSE,scaleHeight=FALSE, alpha=alpha,sigma.sq=sigma.sq,theta0=theta0,theta=theta)
